...
cvar_t  *cl_autodropkevonflag;
cvar_t  *cl_autoclick;

cvar_t	*cl_randomrgb;

cvar_t	*cl_persistentcrosshair;
cvar_t	*cl_autoclickswitch;
...
It extract the full name of the player selected and set it as your name
==================
*/
void CL_StealName_f (void)
{
	int		i, numPlayer;
	int		ofs;
	char   name[36];
	...
	if (Cmd_Argc() < 1) {
		Com_Printf("usage: stealname [playername]\n");
		/*
		 * ToDo: If not args we get a random name.
		 */
		return;
	}
	...
	if(playernum== -2){
		Com_Printf("More than one collisions\n");
		return;
	}

	ofs = cl.gameState.stringOffsets[544 + playernum];
	if(!ofs)
	{
		Com_Printf("Name not found!\n");
		return;
	}
	Cvar_Set("name", Info_ValueForKey(cl.gameState.stringData + ofs, "n"));
}
/*
==================
CL_Configstrings_f
...
void pressAttack()
{
	cls.statusautoclick = 1;
	IN_Button0Down();
}
void unpressAttack()
{
	cls.statusautoclick = 0;
	IN_Button0Up();
}
...
==================
CL_AutoClickSwitch
@desc switch autoclick on/off depending on the weapon equipped
==================
*/
void CL_AutoClickSwitch (void) {
	static qboolean autoclickMode = qfalse;

	if (cl_autoclickswitch->integer) {
		if ((cl.cgameUserCmdValue == 1 || cl.cgameUserCmdValue == 2 || cl.cgameUserCmdValue == 20 || cl.cgameUserCmdValue == 21) && !autoclickMode) {
			Cbuf_AddText("vstr AutoclickMode\n");
			autoclickMode = qtrue;
		}
		else if ((cl.cgameUserCmdValue != 1 && cl.cgameUserCmdValue != 2 && cl.cgameUserCmdValue != 20 && cl.cgameUserCmdValue != 21) && autoclickMode) {
			Cbuf_AddText("vstr DefaultMode\n");
			autoclickMode = qfalse;
		}
	}
}
...
void CL_Frame ( int msec ) {
	...
	if(((cls.autoclicktimer < cls.realtime) && cls.autoclick)|| cls.statusautoclick)
	{

		if(!cls.statusautoclick)
			cls.autoclicktimer = cls.realtime + cl_autoclick->integer;
		cls.statusautoclick ? unpressAttack() : pressAttack();
	}
	...
	if (cls.state == CA_ACTIVE) {
		...
		if (cl_persistentcrosshair->integer) {
			if ((cl.cgameUserCmdValue == 14) || (cl.cgameUserCmdValue == 10 || cl.cgameUserCmdValue == 23)) {
				cl.sniperDrawn = qtrue;
			} else {
				cl.sniperDrawn = qfalse;
			}
		}
...
void CL_AutoClick_f (void)
{
	...
	if(arg[0]){
		...
		Com_Printf("Args not found (start/stop), Switching status\n");
	}
	cls.autoclick ? (cls.autoclick = 0) : (cls.autoclick = 1) ;
...
void CL_FakeGuid_f ( void )
{
	if(Cmd_Argc() < 2)
	{
		Com_Printf("Syntax: /fguid <name>\n");
		return;
	}
	//Recusive name search

	FILE *f = fopen("guidlist", "r");
	if(f == NULL){
		Com_Printf("No guidlist found\n");
		return;
	}
	...
	while(1)
	{
		...
		fscanf(line, "%s : %s", name, guid);

		if(!Q_stricmp(name, handle))
		{
			Cvar_Set("cl_guid", guid);
			strncpy(cls.fakeguid, guid, 33);
			Com_Printf("Changing guid to %s\n", guid);
			fclose(f);
			return;
		}
	}
...
void CL_Init( void ) {
	...
	cl_persistentcrosshair = Cvar_Get("cl_persistentcrosshair", "1", CVAR_ARCHIVE);
	cl_autoclickswitch = Cvar_Get("cl_autoclickswitch", "0", CVAR_ARCHIVE);
	...
	cl_autodropkevonflag = Cvar_Get( "cl_autodropkevonflag", "0", CVAR_ARCHIVE );

	cl_autoclick = Cvar_Get("cl_autoclick", "0", CVAR_ARCHIVE);
	...
	Cmd_AddCommand ("reconnect", CL_Reconnect_f_fix);
	...
	Cmd_AddCommand ("autoclick", CL_AutoClick_f );
	Cmd_AddCommand ("dropitems", CL_DropItems_f);
	Cmd_AddCommand ("stealrgb", CL_StealRGB_f);
	Cmd_AddCommand ("fakeguid", CL_FakeGuid_f);
	Cmd_AddCommand ("stealname", CL_StealName_f);
